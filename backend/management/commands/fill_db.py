    ...
    def handle(self, *args, **options):
        ...
        post_name = [
            {'name': 'Запуск QT на STM32. Часть 2. Теперь с псевдо 3d и тачскрином',
             ...
             'post_text': 'Мы в проекте Embox некоторое время назад запустили Qt на платформе STM32. '
            ...
            {'name': 'История одного видео редактора',
             'short_desc': 'Эта история берет начало у истока 2019 года, когда я заметил, '
                           'что мое бесполезное пребывание в интернете, социальных сетях и смар',
             'post_text': 'Эта история берет начало у истока 2019 года, когда я заметил, что мое бесполезное '
            ...
            {'name': 'Вышел релиз GitLab 13.10 с улучшениями для администраторов и управлением уязвимостями', 'short_desc': 'GitLab 13.10 уже доступен! В этом месяце мы сосредоточили наше внимание на масштабируемости и удобстве управления продуктом, чтобы вы могли итерироват',
             'post_text': 'GitLab 13.10 уже доступен! В этом месяце мы сосредоточили наше внимание на масштабируемости и удобстве управления продуктом, чтобы вы могли итерировать и вводить новшества быстрее, безопаснее и с меньшим количеством проблем. Релиз 13.10 предлагает улучшения администрирования для масштабирования DevOps в вашей организации, проверку целостности пакетов для аварийного восстановления с Geo, автоматизацию управления уязвимостями для большей эффективности и согласованности в обеспечении безопасности и, как и всегда, множество фантастических вкладов от нашего обширного сообщества. Это — лишь некоторые из более чем 40 новых фич и улучшений в данном релизе.', 'hub_category': 'DevOps', 'user_id': 'User_1'},

            {'name': 'Подключаем SSD форм-фактора М2 к материнке, у которой нет разъема М2 и делаем этот SSD системным. Танцы с бубном', 'short_desc': 'Давным-давно, когда в мире жестких дисков только стали появляться твердотельные, я, как все прогрессивное человечество, озаботился приростом производительности посредством этой самой твердотельности носителей. Был куп',
             'post_text': 'Давным-давно, когда в мире жестких дисков только стали появляться твердотельные, я, как все прогрессивное человечество, озаботился приростом производительности посредством этой самой твердотельности носителей. Был куплен недорогой SSD марки Vertex, объемом 120 Гб и с успехом водружен в потроха компьютера. Не помню уже как туда заливалась система (и какая), с трудностями или без, но прирост скорости ощутился конкретно. К диску прилагалась наклейка со словами «My SSD is faster then your HDD», что грело обладателя сего девайса.', 'hub_category': 'Хранение данных', 'user_id': 'User_1'},

            {'name': 'Веб-империя правительства UK: все во имя человека, для блага человека', 'short_desc': 'Цифровизация сейчас в повестке дня едва ли не любого государства. Но все понимают ее по-разному, по-разному ее начинаю',
             'post_text': 'Цифровизация сейчас в повестке дня едва ли не любого государства. Но все понимают ее по-разному, по-разному ее начинают, до разной степени готовности и продуманности доводят и приоритеты расставляют по-разному. Для кого-то и у кого-то цифровизация – это контроль и запреты, для кого-то и у кого-то – удобство и польза каждого отдельного гражданина. С первым вариантом вы все знакомы очно, а о том, как властям Великобритании удается идти по второму пути рассказываем в этой статье на примере подходов к созданию правительственных сайтов.', 'hub_category': 'Usability', 'user_id': 'User_1'},

            {'name': 'От эскиза до релиза: пайплайн регулярного создания контента на примере идеи для оружия от игрока', 'short_desc': 'Огромное количество игр построено на сервисной поддержке, будь то тактический шутер Rainbow Six Siege или большая ролевая World of Warcraft. ',
             'post_text': 'Огромное количество игр построено на сервисной поддержке, будь то тактический шутер Rainbow Six Siege или большая ролевая World of Warcraft. Игроков постоянно вовлекают ивентами, игровыми режимами, картами, персонажами или перками. Но когда в проекте уже сотни и тысячи единиц контента, а релизы ежемесячно — это может стать проблемой для разработчиков.', 'hub_category': 'Работа с 3D-графикой', 'user_id': 'User_1'},

            {'name': 'Как сделать полнотекстовую поисковую машину на 150 строках кода Python',
             'short_desc': 'Полнотекстовый поиск — неотъемлемая часть нашей жизни. '
                           'Разыскать нужные материалы в сервисе облачного хранения документов Scribd, найти фил',
             'post_text': 'Полнотекстовый поиск — неотъемлемая часть нашей жизни. Разыскать нужные материалы в сервисе'
            ...
            {'name': 'Жизнь без AppStore и Google Play: работаем с Huawei Mobile Services и AppGallery',
             'short_desc': 'С конца 2019 Huawei поставляет Android-смартфоны без сервисов Google, в т',
             'post_text': 'С конца 2019 Huawei поставляет Android-смартфоны без сервисов Google, в том '
            ...
            {'name': 'Как реализовать таб-бар с нестандартной кнопкой: CAShapeLayer и UIResponderChain',
             'short_desc': 'Дизайн играет важную роль в мобильном приложении, напрямую влияя на его успех. '
                           'На этапе проектирования интерфейса часто отдается предп',
             'post_text': 'Дизайн играет важную роль в мобильном приложении, напрямую влияя на его успех. '
            ...
            {'name': 'Как сократить стоимость мобильной разработки',
             'short_desc': 'MVP (Minimum Viable Product – «минимально жизнеспособный продукт») – необходим,'
                           ' чтобы понять, как товар или услуга заскакивает аудитории, при максимальных '
                           'затратах на воссоздание. Ей не требуются структурные и дизайнерские излишест',
             'post_text': 'MVP (Minimum Viable Product – «минимально жизнеспособный продукт») – необходим, чтобы '
            ...
            {'name': 'Реализуем бессерверный API с AWS Gateway и Lambda', 'short_desc': 'Без API не обходится ни одно веб-приложение. Для их разработки используются разные методы. Сейчас, например, набирает популярность бессерверный подход — он экономичный, масштабируемый и относительно простой. Как в',
             'post_text': 'Без API не обходится ни одно веб-приложение. Для их разработки используются разные методы. Сейчас, например, набирает популярность бессерверный подход — он экономичный, масштабируемый и относительно простой. Как ведущий провайдер бессерверных вычислений Amazon Web Services (AWS) вносит огромный вклад в бессерверную разработку. Здесь мы обсудим общие концепции реализации API с помощью AWS Lambda и других сервисов AWS.', 'hub_category': 'DevOps', 'user_id': 'User_4'},

            {'name': 'Администрирование Informatica PowerCenter в деталях, часть первая', 'short_desc': 'Привет! Меня зовут Баранов Владимир, и я уже несколько лет администрирую Informatica в «Альфа-Банк». В статье я поделюсь опытом работы с Informatica PowerCenter. IPC это платформа, которая занимается ETL (Extract, Transformation, Lo',
